...
| Tests et stabilisation | Suite de ~695 tests (693 passants) | jusqu'à mi-juin 2026 |
...
La qualité logicielle a reposé sur une suite de tests automatisés substantielle : **près de sept cents tests** (≈ 695), exécutés en environ 170 secondes. À la dernière exécution complète, **693 tests passent et un seul échoue de façon intermittente** ; cet unique échec n'est pas un défaut applicatif mais une **fragilité d'isolation entre tests de bout en bout**, qui partagent un état sur disque : le test **passe systématiquement lorsqu'il est exécuté isolément** (Source interne : tests/). Cette suite combine plusieurs natures de tests, conçues pour couvrir des risques distincts.
...
Le suivi du projet s'est appuyé sur un tableau de bord d'indicateurs objectifs, réévalués au fil des itérations : nombre de tests automatisés passants (**693** sur ≈ 695 en exécution complète, l'unique échec passant isolément), couverture fonctionnelle (**6 pages** opérationnelles), volumétrie du jeu d'apprentissage (**798 fenêtres**, 8 essais), performance du modèle retenu (**RandomForest augmenté, F1-macro 0,918 ± 0,054** sur essai réel), jalons tenus (campagne d'essais, démonstration du 16 juin 2026), et nombre d'incidents de production résolus et figés en tests de non-régression (cf. Tableau 6.1). Ces indicateurs constituent le tableau de bord de pilotage présenté à la Direction (tuteur industriel).
...
Conformément à une démarche d'ingénierie tracée, les principales problématiques techniques rencontrées au cours du développement ont été consignées, avec leur cause, leur date de résolution et le correctif appliqué (identifié par le *commit* Git correspondant). Le tableau ci-dessous en présente une synthèse.
...
*Tableau 6.1 — Suivi des problématiques techniques (index, date, libellé, cause, résolution, solution, référence). Les correctifs sont tracés par leur commit Git.*
...
## 7.2 Cas C1 — formulation lithiée de référence
...
## 7.3 Cas C2 — configuration favorable
...
## 7.4 Cas C3 — détection d'un risque de sur-remplissage
...
## 7.5 Cas C4 — recommandation de l'agent
...
## 7.6 Cas C5 — validation après correction
