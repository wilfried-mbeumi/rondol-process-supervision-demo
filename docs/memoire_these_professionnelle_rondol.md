...
Le volet prédictif procède en deux temps méthodologiques assumés. D'abord, un **championnat de cinq algorithmes supervisés** (régression logistique, SVM, Random Forest, XGBoost, réseau de neurones) départagés non pas sur des partitions aléatoires optimistes, mais par **validation par essai réel non vu** — qui révèle une performance modeste et variable (F1-macro de l'ordre de 0,80), reflet direct du faible nombre d'essais. Ensuite, face à ce déficit, une **augmentation de données documentée, générée à partir de l'échantillon réel** (et non aléatoirement, avec reproduction des imperfections). L'audit de ce second temps constitue l'apport méthodologique central du mémoire : il a révélé une **fuite par ancrage** — le jeu synthétique était généré à partir de la totalité des essais, y compris celui servant de test — qui gonflait artificiellement les scores. Sous le protocole corrigé, où le synthétique est régénéré à partir du seul pli d'entraînement, le gain de l'augmentation **disparaît** et le **Random Forest s'établit à 0,809 ± 0,126** de F1-macro sur essais réels non vus, au lieu des 0,918 initialement mesurés. Il reste le **modèle retenu et déployé**, non pour un score supérieur — les cinq algorithmes sont statistiquement indiscernables — mais pour son interprétabilité et l'importance de variables qu'il fournit ; il est explicitement présenté comme un **indicateur expérimental**, non comme un prédicteur industriel calibré. Les alertes et recommandations opérateur, elles, proviennent de **règles expertes explicables** — la décision n'est jamais confiée au seul modèle statistique. Les valeurs procédé sont présentées comme **nominales et non calibrées industriellement** : l'outil révèle des tendances et compare des configurations, il ne remplace ni l'instrumentation ni le jugement de l'ingénieur. La plateforme repose sur une pile **Streamlit / Python / Supabase (PostgreSQL) / JSON / GitHub / Streamlit Cloud**, validée par 720 tests automatisés (tous passants en suite complète, indépendante de l'ordre d'exécution (vérifiée sur plusieurs graines d'exécution aléatoire)).
...
The predictive component follows two deliberate methodological steps. First, a championship of five supervised algorithms (logistic regression, SVM, Random Forest, XGBoost, a neural network) is judged not on optimistic random splits but under **leave-one-real-trial-out validation**, revealing modest and variable performance (~0.80 macro-F1) — a direct reflection of the small number of trials. Then, since Rondol has no historical trial database, a documented data augmentation is generated from the real sample (never randomly, reproducing even the measurement imperfections). Auditing this second step is the core methodological contribution of this thesis: it uncovered an **anchoring leak** — the synthetic set was generated from all trials, including the one held out for testing — which inflated the reported scores. Under the corrected protocol, where synthetic windows are regenerated from the training fold alone, the benefit of augmentation **vanishes** and the Random Forest settles at **0.809 ± 0.126** macro-F1 on unseen real trials, instead of the 0.918 initially measured. It remains the **retained and deployed** model, not for a superior score — the five algorithms are statistically indistinguishable — but for its interpretability and the feature importances it provides; it is explicitly framed as an **experimental indicator**, not a calibrated industrial predictor. Operator alerts and recommendations, in turn, come from **explainable expert rules** — the decision is never left to the statistical model alone. Process values are presented as nominal and not industrially calibrated: the tool reveals trends and compares configurations rather than replacing instrumentation or engineering judgement. The platform runs on a Streamlit / Python / Supabase (PostgreSQL) / JSON / GitHub / Streamlit Cloud stack, validated by 720 automated tests (all passing, order-independent).
...
Le positionnement de Rondol repose sur deux familles d'extrudeuses, de diamètres de vis 10,5 mm et 21 mm, déclinées en configurations horizontale et verticale. La configuration verticale fait l'objet de brevets européen et américain — un actif de propriété intellectuelle distinctif. Les rapports longueur sur diamètre (L/D) proposés s'échelonnent de 25:1 à 40:1, ce qui autorise une large gamme de profils de vis et de temps de résidence. L'extrudeuse de référence de ce mémoire est la machine bivis 10,5 mm en configuration horizontale, à un rapport L/D de 40:1 : c'est elle qui sert de support à l'ensemble des essais et de la modélisation (Source interne : NOTES_HANDOFF_CLAUDE_CODE.md ; reports/poster_abstract/cases/case_definitions.md).
...
*Tableau 2.1 — Analyse SWOT de Rondol Industrie au regard du projet.*
...
| Tests et stabilisation | Suite de 720 tests (tous passants) | Jusqu'à mi-juin 2026 |
...
L'anticipation des déviances morales possibles (interprétation excessive de valeurs non calibrées comme industrielles, usage de la donnée d'authentification au-delà de sa finalité déclarée) est formalisée dans une charte éthique dédiée en sept principes, présentée en section 8.3 pour rester adossée à la discussion des limites du système qu'elle encadre.
...
La qualité logicielle a reposé sur une suite de tests automatisés substantielle : 720 tests, exécutés en environ 270 secondes. À la dernière exécution complète, les 720 tests passent, indépendamment de l'ordre d'exécution (vérifié sur plusieurs graines d'exécution aléatoire) — une fuite d'état inter-fichiers dans l'isolation des tests ayant été corrigée à sa racine (Source interne : tests/). Cette suite combine plusieurs natures de tests, conçues pour couvrir des risques distincts.
...
Le suivi du projet s'est appuyé sur un tableau de bord d'indicateurs objectifs, réévalués au fil des itérations et présenté au tuteur industriel (Direction) lors des points d'avancement.

| Indicateur | Cible / seuil | Valeur mesurée | Statut |
|---|---|---|---|
| Tests automatisés passants | 100 % de la suite | 720 / 720 | Atteint |
...
| Performance du modèle retenu | F1-macro ≥ 0,85 | 0,809 ± 0,126 (essai réel non vu, protocole corrigé — cf. 5.5.1) | **Non atteint** |
...
| Incidents de production résolus et figés en non-régression | Suivi continu | Cf. Tableau 6.2, Annexe F | Suivi actif |

*Tableau 4.5 — Tableau de bord d'indicateurs de suivi, réévalué à chaque itération et présenté à la Direction (tuteur industriel).*

Un indicateur n'est pas atteint, et il importe de dire pourquoi plutôt que d'ajuster la cible après coup. Le seuil de 0,85 de F1-macro avait été fixé en début de projet, avant de connaître la volumétrie réelle de la campagne d'essais. Il a semblé atteint pendant plusieurs semaines, jusqu'à ce qu'un audit du protocole d'évaluation révèle une fuite d'ancrage dans l'augmentation de données (§ 5.5.1) : le chiffre de 0,918 mesurait en partie la capacité du modèle à retrouver des fenêtres dérivées de l'essai de test lui-même. Sous le protocole corrigé, la performance réelle s'établit à 0,809 sur essai non vu. Cet écart ne traduit pas une défaillance de l'implémentation mais la limite structurelle du projet — huit essais exploitables — déjà identifiée comme risque principal en Partie 4. Il conduit à requalifier le modèle prédictif en **indicateur expérimental** et non en prédicteur industriel, requalification appliquée de bout en bout, y compris dans l'interface.
...
La typologie de cette source unique est simple, ce qui a facilité l'automatisation du traitement. Les douze fichiers sont au format **CSV** encodé UTF-8, séparateur virgule, et totalisent 25,8 Mo pour **310 782 relevés bruts**. Chaque fichier porte trois colonnes de types homogènes : `Timestamp` est une chaîne de caractères horodatée au format ISO JavaScript (« Tue Apr 07 2026 17:09:40 GMT+0000 »), convertie en horodatage typé à l'ingestion ; `Name` est une chaîne catégorielle identifiant le capteur et son unité (« Cast Film Body [°C] »), qui sert de clé de pivot ; `Value` est un flottant en double précision exprimé en degrés Celsius. Aucune donnée à caractère personnel n'y figure : ce sont des mesures d'équipement, ce qui constitue le premier point de contrôle réglementaire du projet — le second portant sur la couche d'authentification, traitée en 8.4 (Source interne : src/config.py ; Essais_07-13_Avril_2026/).
...
Le jeu ainsi obtenu porte le volume d'entraînement de 627 fenêtres réelles à 1 427 (800 fenêtres synthétiques, rééquilibrées à 400 stables / 400 instables). Le protocole d'évaluation retenu est un *Leave-One-Group-Out* **sur les essais réels** : pour chaque essai réel laissé de côté, le modèle est entraîné soit sur les autres essais réels seuls (référence), soit sur ces mêmes essais augmentés du synthétique ; le test est toujours un essai réel non vu, et le synthétique n'apparaît **jamais** en test.

### 5.5.1 Audit du protocole : une fuite par ancrage

Ce protocole, que je pensais suffisant, comporte un défaut que j'ai identifié en auditant mon propre pipeline à la demande de l'encadrement industrielle. Il mérite d'être exposé, car il porte sur le résultat le plus visible de ce mémoire.

Le pool de 800 fenêtres synthétiques était généré **une seule fois, à partir des huit essais réels**, puis injecté dans le pli d'entraînement de chaque itération. À chaque fold, l'essai censé être exclu avait donc doublement contribué à l'entraînement de façon indirecte : ses fenêtres avaient servi de **points d'ancrage** au tirage par bootstrap, et ses valeurs avaient alimenté les **écarts-types de classe** pilotant l'amplitude du jitter. L'essai de test n'était jamais vu *directement*, mais son empreinte statistique était présente dans les données d'entraînement. Il s'agit d'une fuite par ancrage — une forme discrète de contamination, invisible sur une lecture rapide du code, puisque le fold de test ne contient effectivement aucune fenêtre synthétique.

La correction consiste à rendre la génération **dépendante du fold** : à chaque itération, le pool synthétique est intégralement régénéré à partir des seuls essais d'entraînement, avec le même algorithme, le même volume et la même graine. Pour établir que seule la source d'ancrage change, et non la méthode de génération, le générateur corrigé a été confronté au pool publié : alimenté par le support d'ancrage complet, il le reproduit **à l'identique bit à bit sur les 87 variables**.

| Modèle | Sans augmentation | Augmentation globale (protocole initial) | Augmentation par fold (protocole corrigé) |
|---|:--:|:--:|:--:|
| Régression logistique | 0,799 ± 0,163 | 0,860 ± 0,111 | 0,809 ± 0,173 |
| SVM (noyau RBF) | 0,805 ± 0,171 | 0,868 ± 0,089 | **0,824 ± 0,139** |
| **Random Forest** | 0,809 ± 0,176 | 0,918 ± 0,054 | 0,809 ± 0,126 |
| XGBoost | 0,757 ± 0,213 | 0,900 ± 0,073 | 0,801 ± 0,159 |
| Réseau de neurones (MLP) | 0,778 ± 0,217 | 0,862 ± 0,118 | 0,781 ± 0,164 |

*Tableau 5.2 — Effet de l'augmentation en validation Leave-One-Group-Out sur essais réels, selon que le pool synthétique est ancré sur la totalité des essais ou sur le seul pli d'entraînement. Moyennes sur les six essais évaluables (les essais 32 et 42, intégralement stables, ne permettent ni F1-macro interprétable ni ROC-AUC). Source interne : reports/AI_thesis_results/block_2_model_augmentation/.*

L'enseignement est net, et il n'est pas celui que j'attendais. **Le gain attribué à l'augmentation était très majoritairement un artefact de la fuite** : sur le Random Forest, il passe de +0,109 à −0,001. La réduction de variance annoncée se réduit elle aussi (écart-type de 0,176 à 0,126, et non de 0,187 à 0,054). Aucun modèle n'atteint 0,85 sous le protocole corrigé, et les cinq restent groupés entre 0,78 et 0,82 — soit, compte tenu d'écarts-types de 0,13 à 0,17 sur six folds, **des performances statistiquement indiscernables**. L'augmentation par ancrage, telle que je l'ai conçue, ne compense donc pas le déficit de données réelles : elle en produisait surtout l'illusion.

Je choisis d'exposer ce résultat plutôt que de le taire, pour trois raisons. La première est déontologique : un chiffre de 0,918 présenté comme la performance d'un modèle déployé aurait constitué une sur-promesse envers Rondol. La deuxième est méthodologique : cette fuite illustre mieux que n'importe quel développement réussi le point central de la Partie 5 — la performance d'un modèle est une propriété du **protocole d'évaluation** autant que de l'algorithme. La troisième est pratique : l'ensemble des prédictions individuelles, métriques par fold et scripts de génération est livré, ce qui rend chaque chiffre de ce mémoire recalculable et contestable par un tiers.
...
Le protocole de validation constitue le point méthodologique le plus important de cette partie, car il conditionne la crédibilité de toute conclusion. Le principe directeur est la **séparation par essai** : les fenêtres d'un même essai étant fortement autocorrélées, les répartir aléatoirement entre entraînement et test laisserait fuir de l'information et gonflerait artificiellement les performances. Trois dispositifs ont donc été combinés. Le **GroupShuffleSplit** répartit les essais entiers entre entraînement et test, garantissant qu'aucun essai n'est simultanément en entraînement et en test. La validation **Leave-One-Group-Out** évalue le modèle en excluant tour à tour un essai entier, fournissant une estimation exhaustive de la variabilité inter-essais. Enfin, le **décalage de la cible** d'une fenêtre prévient la fuite temporelle au sein d'un même essai.

Ce choix appelle une explication au regard du référentiel, qui demande un jeu d'entraînement représentant **au moins 70 % des données initiales**. La règle est satisfaite par le protocole de référence, mais pas par le premier jalon du projet, et il faut dire pourquoi. Les huit essais ont des volumétries très inégales : le seul essai 1 fournit 250 des 627 fenêtres, soit 40 % du jeu. Réserver trois essais au test laisse donc 287 fenêtres à l'entraînement, soit **45,8 %** — en deçà du seuil, alors même que la partition demandée au tirage était de 70 % des essais. C'est la conséquence directe du refus de découper à l'intérieur d'un essai : seule une séparation par groupe empêche la fuite décrite plus haut, et elle interdit d'ajuster finement la proportion de lignes.

Sous le protocole de référence, en revanche, la règle est tenue. En Leave-One-Group-Out, chaque itération entraîne le modèle sur sept des huit essais, soit **87,5 % des fenêtres en moyenne** ; sept des huit plis dépassent le seuil, avec des parts d'entraînement comprises entre 81,7 % et 95,4 %. L'unique exception est le pli qui écarte l'essai 1 : parce que cet essai concentre à lui seul 40 % des données, l'entraînement y retombe à 60,1 %. Cette exception est structurelle, non méthodologique — elle disparaîtrait avec une campagne d'essais mieux équilibrée, ce qui rejoint la limite principale exposée en 5.10. Les résultats publiés dans ce mémoire sont ceux du protocole Leave-One-Group-Out ; le tableau du holdout unique est conservé comme trace du premier jalon, et non comme référence.
...
*Tableau 5.3 — Performances comparées sur le jeu de test (fenêtre 60 s). En validation croisée 5-plis sur l'entraînement, le Random Forest atteint un F1-macro de 0,935 ± 0,029 et une AUC de 0,976 ± 0,021. Sous validation stricte par essai (Leave-One-Group-Out), le F1-macro réaliste se situe à 0,77–0,79 (cf. 5.7).*

Quatre enseignements ressortent. D'abord, Random Forest, SVM et la baseline de *deep learning* (MLP) offrent des performances très proches et nettement supérieures à XGBoost sur ce jeu (F1-macro de 0,92 à 0,93). Ensuite, point décisif pour le choix d'architecture, le réseau de neurones n'apporte aucun gain décisif justifiant sa complexité et sa moindre interprétabilité : sur un jeu tabulaire de cette taille, les méthodes classiques restent au moins aussi performantes, ce qui confirme le choix de ne pas recourir au *deep learning* en production. Il faut toutefois rappeler que ces chiffres élevés proviennent de partitions **optimistes** : le départage réel entre modèles est opéré en validation par essai non vu, sous le protocole d'augmentation corrigé (§5.5.1), où les cinq algorithmes se révèlent statistiquement indiscernables et où le **Random Forest** est retenu sur des critères de stabilité et d'interprétabilité plutôt que de score (§5.9). Enfin, comme souligné en 5.7, les chiffres d'exactitude « bruts » doivent être lus à la lumière de la validation stricte, qui ramène les performances réalistes autour de 0,78–0,82 de F1-macro.

## 5.9 Choix du modèle retenu et intégré au prototype

Le choix du modèle repose sur un **championnat honnête** : les cinq algorithmes sont départagés non pas sur les partitions aléatoires (optimistes, cf. 5.7), mais sur la validation par essai réel non vu, sous le protocole d'augmentation corrigé décrit en 5.5.1. Sous ce protocole, aucun modèle ne se détache : le SVM obtient le meilleur F1-macro (0,824 ± 0,139), devant la régression logistique (0,809 ± 0,173), le Random Forest (0,809 ± 0,126), XGBoost (0,801 ± 0,159) et le réseau de neurones (0,781 ± 0,164). Ces écarts, de l'ordre de 0,02, sont **très inférieurs aux écarts-types inter-essais**, de l'ordre de 0,13 à 0,17 sur six folds : les cinq algorithmes sont statistiquement indiscernables sur ce jeu de données. Prétendre qu'un modèle « gagne » ici serait sur-interpréter le bruit de mesure.

Le Random Forest est néanmoins retenu, et le critère de décision doit être énoncé explicitement puisqu'il n'est **pas** la performance brute. Trois arguments le fondent. D'abord la **stabilité inter-essais** : à performance moyenne équivalente, il présente le plus faible écart-type des cinq (0,126), c'est-à-dire le comportement le plus régulier d'un essai à l'autre — critère décisif quand la robustesse prime sur la performance de pointe. Ensuite l'**interprétabilité** : il fournit nativement une importance de variables, exploitée en Partie 6 pour relier les alertes du modèle aux grandeurs procédé que l'opérateur comprend ; ni le SVM à noyau ni le réseau de neurones n'offrent cette lisibilité, rédhibitoire dans un contexte industriel où une alerte doit pouvoir être justifiée. Enfin la **tolérance native aux données manquantes**, qui évite d'ajouter une étape d'imputation au moment de l'inférence sur un flux capteur où les coupures sont fréquentes.

Ce raisonnement illustre un point que je retiens du projet : le choix d'un modèle en contexte industriel ne se réduit pas à la maximisation d'une métrique, surtout lorsque celle-ci n'est pas significativement discriminante.
...
La démarche d'apprentissage automatique conduite dans ce projet établit, à partir de données d'essais réelles mais peu nombreuses, une première capacité de prévision de la stabilité thermique du régime d'extrusion, validée selon des protocoles stricts qui en donnent une mesure réaliste et non flattée. Face au déficit de volume, une augmentation de données documentée à partir de l'échantillon a été construite ; son audit a révélé une fuite par ancrage dont la correction annule le gain apparent (§ 5.5.1). Au terme d'un championnat évalué par essai réel non vu et sous protocole corrigé, les cinq algorithmes se révèlent statistiquement indiscernables autour de 0,80 de F1-macro ; le Random Forest est retenu comme modèle prédictif déployé (0,809 ± 0,126) sur des critères de stabilité, d'interprétabilité et de tolérance aux données manquantes, et non sur une supériorité de score. La logique de recommandation reste, elle, assurée par des **règles expertes explicables**. La conclusion opérationnelle est que le facteur limitant n'est ni l'algorithme ni la méthode, mais le nombre d'essais : passer d'un indicateur expérimental à un prédicteur industriel exigera de nouvelles campagnes, non un modèle plus sophistiqué.
...
*Tableau 6.1 — Comparaison du temps d'exécution des requêtes entre table non optimisée et table optimisée (accélération ≈ ×1000). Le principe, index B-tree vs balayage, est identique sous PostgreSQL/Supabase.*
...
La fiabilité de la plateforme s'appuie sur une couverture de tests substantielle : la suite compte **720 tests** (dont onze dédiés à l'authentification), pour une durée d'environ 270 secondes ; à la dernière exécution complète, **les 720 passent, indépendamment de l'ordre d'exécution** — propriété vérifiée sur plusieurs graines d'exécution aléatoire après correction d'une fuite d'état inter-fichiers dans l'isolation des tests (Source interne : tests/ ; CLAUDE.md). Cette batterie ne se limite pas à la vérification fonctionnelle des pages ; elle protège également les invariants structurels du système, au premier rang desquels l'unicité du module backbone, contrôlée par les tests dédiés au singleton d'import. La séparation entre un sous-ensemble de tests purs et rapides, exécutable en boucle de développement courte, et les tests d'interface plus lourds reposant sur `streamlit.testing.v1.AppTest` permet un cycle de validation efficace sans sacrifier la couverture de bout en bout.

Le déploiement s'effectue selon une chaîne intégrée : le code est hébergé sur GitHub, l'application est servie par Streamlit Cloud et accessible publiquement à l'adresse <https://rondol-process-supervision-demo.streamlit.app>, et la persistance durable de l'état validé est assurée par Supabase, conformément à l'architecture de persistance décrite en 6.6. Cette combinaison répond à l'objectif d'un outil réellement démontrable et accessible à distance par le client et le tuteur industriel, tout en contournant la limitation du disque éphémère grâce au backend durable et à l'auto-réparation déterministe. L'ensemble traduit une exigence constante du projet : préférer la robustesse et la cohérence d'intégration à l'accumulation de fonctionnalités, afin de présenter devant le jury comme devant Rondol Industrie un système crédible, testé et opérationnel.

Au-delà de l'application, le référentiel attend un livrable d'analyse de données sous forme de notebook. Le fichier `notebooks/notebook_application_rondol.ipynb` documente la chaîne complète en quatorze cellules exécutables : moteur procédé, qualité et sécurisation des données, comparaison des temps de requête entre table indexée et non indexée, championnat des modèles, audit de la fuite par ancrage avec les trois protocoles d'augmentation, validation externe, et exécution du moteur de règles sur le code réellement déployé. Sa rejouabilité a été vérifiée sur un clone vierge du dépôt : l'exécution complète prend quatorze secondes et ne produit aucune erreur. Les deux jeux de données volumineux qui ne sont pas versionnés — la base consolidée et les séries brutes fusionnées — font l'objet d'une dégradation explicite : la cellule concernée annonce son abandon et indique la commande de régénération, au lieu de laisser une trace d'erreur. Le notebook est joint au ZIP du projet, seul livrable à contenir également les mesures de la campagne d'avril (Source interne : notebooks/notebook_application_rondol.ipynb ; scripts/build_notebook_application.py).
...
*Tableau 7.1 — Consignes thermiques par zone retenues pour le protocole de démonstration.*
...
*Tableau 7.2 — Synthèse des cinq cas de démonstration : score, probabilité de stabilité et observation clé.*
...
L'accessibilité numérique a fait l'objet d'un audit dédié en fin de projet, dont les constats ont été corrigés puis figés en tests. Trois points méritent d'être rapportés, y compris parce que le premier a démenti ce que je croyais acquis. D'abord les contrastes : le thème sombre de l'application a été mesuré au regard du critère WCAG 2.1 AA, qui exige un rapport minimal de 4,5:1. Les quatre couples de couleurs utilisés le respectent largement — texte sur fond principal 18,39:1, texte sur fond secondaire 16,98:1, vert primaire sur fond principal 6,91:1 et sur fond secondaire 6,38:1. Ensuite les libellés de formulaire : je pensais que les quatre champs dont l'étiquette est masquée visuellement (`label_visibility="collapsed"`) étaient dépourvus de nom accessible. L'inspection du DOM rendu montre l'inverse — Streamlit émet malgré tout l'attribut `aria-label` et l'élément `<label>` associé. Le défaut supposé n'existait pas.

Le manque réel se situait ailleurs, dans le seul bloc d'interface écrit à la main : le schéma du profil de vis, dessiné en HTML et CSS, que Streamlit ne peut donc pas instrumenter à ma place. Sans nom accessible, il restait entièrement muet pour un lecteur d'écran. Le conteneur est désormais exposé comme une image unique (`role="img"`) portant un résumé du profil — nombre d'éléments placés, de positions et de zones — le détail position par position restant lu dans le tableau des éléments placés situé sous le schéma. L'arbre d'accessibilité du navigateur a par ailleurs révélé un second défaut, inverse du premier : les soixante-neuf positions sans élément y figuraient toutes, ce qui aurait fait énoncer « arbre nu » soixante-neuf fois de suite. Elles sont désormais marquées `aria-hidden`, conformément au principe qu'un contenu décoratif ne doit pas encombrer la restitution vocale. Trois infobulles figées en français, qui fuitaient en mode anglais, ont été traitées au passage. Quinze tests automatisés verrouillent l'ensemble : présence du nom accessible, fidélité du résumé au profil réellement configuré, masquage des positions vides, absence de fuite linguistique dans les deux sens, et vérification des quatre rapports de contraste (Source interne : tests/test_accessibilite_rendu_vis.py ; app/screw_render.py).

Ce chantier n'est pas clos pour autant, et il serait malhonnête de le présenter comme tel. La hiérarchie sémantique complète des pages — niveaux de titres, points de repère — n'a pas été reprise, et l'audit de contraste porte sur la palette du thème, non sur chaque composant en situation. La navigation au clavier repose sur le comportement natif de Streamlit, que je n'ai pas testé avec un lecteur d'écran réel. Ces réserves relèvent d'une mise à disposition élargie ; le prototype livré, lui, ne comporte plus de contenu graphique muet ni de contraste insuffisant.
...
Ce que je retiens de ce travail tient en quelques points. J'ai d'abord construit un jumeau numérique crédible : une logique métier ancrée dans la vraie géométrie de la vis (81 positions), un modèle supervisé nourri par douze capteurs de la campagne d'avril 2026, un agent explicable qui recommande. Mais le cœur méthodologique, c'est d'avoir affronté le manque de données plutôt que de le masquer. Mon championnat de cinq modèles, jugé sur des essais réels jamais vus, a donné des résultats modestes et instables — un F1-macro autour de 0,80, l'exacte mesure de mes huit essais. J'ai cru un temps avoir débloqué la situation par l'augmentation de données : elle semblait porter le Random Forest à 0,918 et diviser sa variance par plus de trois. C'est en auditant mon propre protocole, à la demande de l'encadrement industrielle, que j'ai découvert que ce gain était un artefact : mes fenêtres synthétiques étaient ancrées sur la totalité des essais, y compris celui que je gardais pour tester. Une fois la génération confinée au pli d'entraînement, le gain disparaît et le modèle retombe à 0,809.

Cette découverte est, rétrospectivement, ce que ce projet m'a le plus appris. Non pas techniquement — la correction elle-même tient en une régénération par fold — mais professionnellement. J'aurais pu livrer 0,918 : le chiffre était reproductible, le code fonctionnait, le protocole paraissait rigoureux et personne ne me l'avait contesté. Ce qui l'a mis en défaut, c'est une checklist de garanties méthodologiques, point par point, à laquelle j'ai répondu honnêtement plutôt que par la réponse attendue. J'en retiens qu'un résultat qui va dans le sens de ce qu'on espère mérite un examen plus sévère, pas plus indulgent, et qu'annoncer soi-même une correction vaut mieux que la voir découverte par un tiers. J'ai aussi tenu à séparer clairement les rôles : le modèle prédit, les règles expertes recommandent, l'humain décide. Et je n'ai jamais caché l'écart entre validation naïve et validation stricte — je le revendique comme la marque de la rigueur. Le tout repose sur une persistance durable auto-réparatrice (Supabase, avec repli fichier puis JSON local) et plus de sept cents tests automatisés (Source interne : app/persistence.py ; tests/).
...
Le jeu de données complet retenu pour le modèle de production (fenêtre 60 secondes) comporte **87 variables brutes** par fenêtre, obtenues en combinant sept statistiques par capteur sur les douze voies, augmentées de trois variables croisées entre capteurs (Source interne : src/features.py ; src/config.py).

[À COMPLÉTER : dictionnaire exhaustif des 87 variables dérivées, avec pour chacune le nom exact, la définition, l'unité, le capteur source et le rôle dans le modèle.]
...
*Tableau 7.3 — Synthèse des cas tests C1 à C5. La séquence C3 → C4 → C5 illustre le parcours complet : détection d'une instabilité (C3), recommandation chiffrée de l'agent (C4), retour à un régime stable après correction (C5). Source interne : reports/poster_abstract/cases/case_definitions.md.*
...
## Annexe F — Suivi des problématiques techniques (Tableau 6.2)
...
| 2 | Juin 2026 | Désynchronisation de l'état entre les pages | Sources d'état multiples (widgets vs snapshot) | Juin 2026 | Snapshot `applied_state` comme source unique, `hydrate_session_from_applied` sur les pages applicatives | `8f109ea` |
...
*Tableau 6.2 — Suivi des problématiques techniques ; correctifs tracés par commit Git.*
